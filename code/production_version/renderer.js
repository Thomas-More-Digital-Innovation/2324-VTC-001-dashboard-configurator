...
async function constructPage() {
  try {
    ...
    for (let i = 1; i <= graphlist.length; i++) {
      ...
      const graphChangedNames = graphsAttributeList[parseInt(graphId)].changedNames;
      switch (graphType) {
        case "bar":
          SimpleBarChart(
            graphId,
        ...
        case "scatterplot":
          ScatterPlotChart(graphId,
            ...
            showLegenda,
            legendaView,
            ScatterPlotCsvParser(content, graphTargetColumn, graphTimeColumn)
          );
          break;
        case "scatteraxis":
          ScatterAxisChart(
            ...
            showLegenda,
            legendaView,
            graphsAttributeList[parseInt(graphId)].textLeft,
            graphsAttributeList[parseInt(graphId)].textRight,
        ...
        case "gauge":
          GaugeChart(graphId,
            ...
            showLegenda,
            legendaView,
          GaugeChartCsvParser(content, graphTargetColumn));
          break;
        case "fillshape":
          ...
          switch (graphs[l].getAttribute("type")) {
            ...
            case "scatteraxis":
              graphsAttributeList.push({
                ...
                legendaView: graphs[l].getAttribute("legendaView"),
                showLabels: graphs[l].getAttribute("showLabels"),
                textLeft: graphs[l].getAttribute("textLeft"),
                textRight: graphs[l].getAttribute("textRight"),
                changedNames: changedNames,
              });
              break;
            case "gauge":
...
// --- Simple Bar Chart --- //
function SimpleBarChartCsvParser(content, targetColumn, changedNames) {
  ...
  // Iterate through the data to collect unique values and their counts
  for (let i = 1; i < rows.length; i++) {
    const value = rows[i][targetColumnIndex];
    const changedValue = findChangedName(value, changedNames) || value; // Find the changed name if applicable
    if (valueCounts[changedValue] === undefined) {
      // If the value is encountered for the first time, initialize its count to 1
      valueCounts[changedValue] = 1;
    } else {
      // If the value is encountered again, increment its count
      valueCounts[changedValue]++;
    }
  }

  // Extract unique values and their counts into separate arrays
...
// --- Simple Line Chart --- //
function SimpleLineChartCsvParser(content, targetColumn, changedNames) {
  // Parse CSV content into an array of rows
  const rows = content.trim().split('\n').map(row => row.trim().split(';'));
...
// --- Stacked Line Chart --- //
function MultipleLineChartCsvParser(content, timeColumnName, targetColumnName) {
  const rows = content.split("\n");
  const headers = rows[0].split(";").map((header) => header.trim());
  const data = {};
...
function MultipleLineChart(id, width,
  height, title, subtitle, showLegenda, legendaView, data) {
  return new Promise((resolve) => {
    let chart = echarts.init(document.getElementById(id), {
      width: width,
...
function ScatterPlotCsvParser(content, targetColumn, timeColumn) {
  ...
  rows.forEach(row => {
      ...
      // Push an array containing target and time values to the result array
      result.push([targetValue, timeValue]);
  });
...
function ScatterAxisChart(id, width, height, title, subtitle, showLegenda, legendaView, textLeft, textRight, data) {
  function generateAxisLabels() {
    let labels = [];
    for (let i = 0; i < 100; i++) {
      if (i === 7) {
        labels.push(textLeft);
      } else if (i === 92) {
        labels.push(textRight);
      } else {
        labels.push('');
      }
    }
...
// --- Gauge Chart --- //
function GaugeChartCsvParser(content, targetColumn) {
  // Split the CSV content into rows
  const rows = content.trim().split('\n');
  ...
  rows.forEach(row => {
      ...
      // Extract the target value from the row
      const targetValue = parseFloat(values[targetIndex].replace(',', '.')); // Replace comma with dot
      
      // Add the target value to the total
      total += targetValue;
      count++;
  });
...
function GaugeChart(
  ...
  return new Promise((resolve) => {
    ...
    const color1 = dashboardColorList[1];
    const color2 = interpolateHex(dashboardColorList[0], dashboardColorList[1], 0.25);
    const color3 = interpolateHex(dashboardColorList[0], dashboardColorList[1], 0.5);
    const color4 = interpolateHex(dashboardColorList[0], dashboardColorList[1], 0.75);
...
function interpolateHex(color1, color2, ratio) {
